...
function newScoreSheet() {
  ...
  // Find the "🔴 System/Rules" sheet and place new tab to its right
  var insertIndex = 1; // default fallback
  var sheets = ss.getSheets();
  for (var i = 0; i < sheets.length; i++) {
    if (sheets[i].getName() === "🔴 System/Rules") {
      insertIndex = sheets[i].getIndex() + 1; // Place to the right of System/Rules
      break;
    }
  }
  
  var displayName = '🟢 ' + Utilities.formatDate(new Date(), ss.getSpreadsheetTimeZone(), "MM-dd-yyyy");
